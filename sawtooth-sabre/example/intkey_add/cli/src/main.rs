...
fn run() -> Result<(), error::CliError> {
    ...
    let matches = clap_app!(myapp =>
        (name: APP_NAME)
        (version: VERSION)
        (about: "Sawtooth Intkey add CLI")
        (@setting SubcommandRequiredElseHelp)
        (@subcommand add =>
            (about: "add two intkey values together")
            (@arg name_a: +required +takes_value "Intkey key to store multiplied value")
            (@arg name_b:  +takes_value +required "Intkey key for the first value to add")
            (@arg name_c: +takes_value +required "Intkey key for the second value to add")
    ...
    if let Some(add_matches) = matches.subcommand_matches("add") {
        let name_a = add_matches.value_of("name_a").unwrap();
        let name_b = add_matches.value_of("name_b").unwrap();
        let name_c = add_matches.value_of("name_c").unwrap();
        let output = add_matches.value_of("output").unwrap_or("");
        let key_name = add_matches.value_of("key");
        let url = add_matches
            .value_of("url")
            .unwrap_or("http://localhost:8008/");

        add::do_add(name_a, name_b, name_c, output, key_name, &url)?;
    }
