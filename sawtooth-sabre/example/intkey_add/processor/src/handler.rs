...
pub struct IntkeyaddTransactionHandler {
    family_name: String,
    family_versions: Vec<String>,
    namespaces: Vec<String>,
}

impl IntkeyaddTransactionHandler {
    #[allow(clippy::new_without_default)]
    pub fn new() -> IntkeyaddTransactionHandler {
        IntkeyaddTransactionHandler {
            family_name: "intkey_add".to_string(),
...
impl TransactionHandler for IntkeyaddTransactionHandler {
    ...
    fn apply(
        ...
        let orig_value_b: u64 = match state.get(payload.get_name_b()) {
            ...
            Ok(None) => {
                return Err(ApplyError::InvalidTransaction(String::from(
                    "add requires a set value for name_b",
                )));
            }
            Err(err) => return Err(err),
        };
        ...
        let orig_value_c: u64 = match state.get(payload.get_name_c()) {
            ...
            Ok(None) => {
                return Err(ApplyError::InvalidTransaction(String::from(
                    "add requires a set value for name_c",
                )));
            }
            Err(err) => return Err(err),
        };
