...
const INTKEY_ADD_DEF: &str = "/project/example/intkey_add/intkey_add.yaml";
...
// Path to a payload to add intkey value B and C and store in A.
const GOOD_PAYLOAD: &str = "/project/integration/payloads/A_B_C_payload";
// Path to a payload to add intkey value C and nonexisties and store in A.
...
fn pike_setup(signer: &str) -> Result<(), TestError> {
    ...
    // 6) Add read and write perms for cad11d for intkey_add
    // 7) Add read and write perms for 00ec03 for intkey_add
...
/// The following test tests the Sabre Cli, Sabre Transaction Processor and the Intkey add
...
fn test_sabre() {
    ...
    //      Inputs: 1cf126
    //      Outputs: 1cf126
    //      Payload: A payload that tries to add B and C together and set in A.
    //
    // Result: Invalid Transaction, Contract does not exist
    let response = match sabre_cli(
        "exec --contract intkey_add:1.0 --payload ".to_string()
            + &GOOD_PAYLOAD
    ...
    //      Inputs: 1cf126
    //      Outputs: 1cf126
    //      contract: The compiled intkey_add wasm contract.
    //
    // Result: Invalid Transaction, The Contract Registry does not exist
    let response = match sabre_cli("upload -f ".to_string() + &INTKEY_ADD_DEF) {
        Ok(x) => x,
        Err(err) => panic!(format!("No Response {}", err)),
    };
    assert!(response["data"][0]["status"] == "INVALID");
    ...
    //
    // Result: Committed.
    let response = match sabre_cli("cr --create intkey_add --owner ".to_string() + &signer) {
        Ok(x) => x,
        Err(err) => panic!(format!("No Response {}", err)),
    };
    assert!(response["data"][0]["status"] == "COMMITTED");
    ...
    //      Namespace: 1cf126
    //      Contract_name: intkey_add
    //      Read: true
    //      Write: true
    //
    // Result: Committed
    let response = match sabre_cli("perm 1cf126 intkey_add --read --write".to_string()) {
        Ok(x) => x,
        Err(err) => panic!(format!("No Response {}", err)),
    };
    assert!(response["data"][0]["status"] == "COMMITTED");
    ...
    //
    // Result: Committed
    let response = match sabre_cli("perm cad11d intkey_add --read --write".to_string()) {
        Ok(x) => x,
        Err(err) => panic!(format!("No Response {}", err)),
    };
    assert!(response["data"][0]["status"] == "COMMITTED");
    ...
    //
    // Result: Committed
    let response = match sabre_cli("perm 00ec03 intkey_add --read --write".to_string()) {
        Ok(x) => x,
        Err(err) => panic!(format!("No Response {}", err)),
    };
    assert!(response["data"][0]["status"] == "COMMITTED");
    ...
    //      Inputs: 1cf126
    //      Outputs: 1cf126
    //      Payload: A payload that tries to add Bad and C together and set in A.
    //
    // Result: Invalid Transaction, Wasm contract returned invalid transaction
    let response = match sabre_cli(
        "exec --contract intkey_add:1.0 --payload ".to_string()
